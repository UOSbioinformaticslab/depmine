...
# 29/01/2024 - All gene names in muts,Encyclopedia.cnvs and exprnow made to conform to HUGO reference, so larger numbers available
...
# LHP
    ...
    default_dep_dat_file = 'CRISPRGeneDependency.csv'
    default_cell_info_file = 'Model.csv'
    default_mut_dat_file = 'OmicsSomaticMutations.csv'
    default_phys_chro_file = 'c1.all.v2023.1.Hs.json.txt' # from GSEA
    default_HUGO_file = 'hgnc_complete_set.tsv'
    default_cnv_dat_file = 'OmicsCNGene.csv'
    default_fus_dat_file = 'OmicsFusionFiltered.csv'
    default_expr_dat_file = 'OmicsExpressionProteinCodingGenesTPMLogp1.csv'
    default_expr_thresh_file = 'ExpressionThresholds.csv'
    default_signatures_file = 'Assignment_Solution_Activities.tsv'
    default_pathways_file = 'c2.cp.reactome.v2023.2.Hs.symbols.gmt'
    default_GeneDatabase_file = 'AB_GeneDatabaseExpr2023Q2'
    default_CellDatabase_file = 'AB_CellDatabaseExpr2023Q2'
    ...
    # default_cell_info_file = ""
    # ModelFile = 'Model.csv'
    # default_mut_dat_file = ""
    # OmicsFile = 'OmicsSomaticMutations.csv'
    # default_phys_chro_file = ""
    # ChroFile = 'c1.all.v2023.1.Hs.json.txt'
    # default_HUGO_file = ""
    # HgncFile = 'hgnc_complete_set.tsv'
    # default_cnv_dat_file = ""
    # OmicsCNGeneFile = 'OmicsCNGene.csv'
    # default_fus_dat_file = ""
    # FusionFilteredFile = 'OmicsFusionFiltered.csv'
    # default_expr_dat_file = ""
    # ExpressionFile = 'OmicsExpressionProteinCodingGenesTPMLogp1.csv'
    # default_file = ""
    # ExpressionThresholdsFile = 'ExpressionThresholds.csv'
    # GOFFile = 'Final_GOF_all_table.csv'
    # default_signatures_file = ""
    # AssignmentFile = 'Assignment_Solution_Activities.tsv'
    # default_pathways_file = ""
    # c2File = 'c2.cp.reactome.v2023.2.Hs.symbols.gmt'
    # default_GeneDatabase_file = ""
    # GeneDatabase_file = 'AB_GeneDatabaseExpr2023Q2'
    # default_CellDatabase_file = ""
    # CellDatabase_file = 'AB_CellDatabaseExpr2023Q2'
...
class Functions:
    ...
    # combine into mutation
        ...
        qGoFs = muts[muts['ModelID'].isin(q) & Encyclopedia.muts['LikelyGoF'] == True] \
                    [['HugoSymbol','ProteinChange']]
    ...
    def WhichSigType(s):
        r = 'UNASSIGNED'
        for i in Encyclopedia.SigGroups:
            if s in Encyclopedia.SigGroups[i]:
                r = i
        return r
    ...
    # get counts for each signature in all cells in set
        ...
        sig_count =   dict(Encyclopedia.sigs[Encyclopedia.sigs['SAMPLES'].isin(s)].sum().loc[Encyclopedia.sigs.columns[1:-1]])
        ...
        sgs = {}
        for i,nm in enumerate(Encyclopedia.SigGroups):
            sgs[nm] = i
    ...
    def PlotCellType(p):
        ...
        nincl = 0
        for i in p:
            c = Encyclopedia.info.loc[Encyclopedia.info['ModelID'] == i]
            if len(c) > 0:
    ...
    def GetOncotreePrimaryDisease():
        return dict(sorted(Counter(list(Encyclopedia.info['OncotreePrimaryDisease'])).items(),
                        key = lambda x: x[1], reverse = True))   
        
    def GetParams():
        
        t = input(f'Dependency threshhold [{Encyclopedia.thresh}] : ').strip()
        if Functions.isFloat(t) and float(t) >=0.0 and float(t) <=1.0:
            Encyclopedia.thresh = float(t)
        print(f'Using threshold of {Encyclopedia.thresh}')
            
        t = input(f'Minimun mutational frequency [{Encyclopedia.frac_limit}] : ').strip()
        if Functions.isFloat(t) and float(t) >=0.0 and float(t) <=1.0:
            Encyclopedia.frac_limit = float(t)
        print(f'Using minimun mutational frequency of {Encyclopedia.frac_limit}')
            
        t = input(f'Maximum p value for hit [{Encyclopedia.p_limit}] : ').strip()
        if Functions.isFloat(t) and float(t) >=0.0 and float(t) <=1.0:
            Encyclopedia.p_limit = float(t)
        print(f'Using maximum p value for hit of {Encyclopedia.p_limit}')
        
        t = input(f'Minimum Cohen d value for hit [{Encyclopedia.cohen_min}] : ').strip()
        if Functions.isFloat(t) and float(t) >=0.0 and float(t) <=1.0:
            Encyclopedia.p_limit = float(t)
        print(f'Using minimum Cohen d value for hit of {Encyclopedia.cohen_min}')

# LHP        
        t = input(f'Minimum mutational signature fraction [{Encyclopedia.sig_thresh}] : ').strip()
        if Functions.isFloat(t) and float(t) >=0.0 and float(t) <=1.0:
            Encyclopedia.p_limit = float(t)
        print(f'Using minimum mutational signature fraction of {Encyclopedia.sig_thresh}')
        
        
    # # Get factor to trim Null sets by 
        
    #     t = input(f'Null set trim factor [{null_trim}] : ').strip()
    #     if Functions.isFloat(t) and float(t) >=0.0 :
    #         null_trim = float(t)
    ...
    def prof_load(fname):
        ...
        while True:
            ...
            if t == '':
                ...
                break
            Encyclopedia.profiles[s[:-1].strip().upper()] = t[:-1].strip()
        f.close()
        return True

    def prof_save(fname):
        f = open(fname,'w')
        for n in Encyclopedia.profiles:
            f.write(n.upper() +'\n')
            f.write(Functions.prof_flatten(Encyclopedia.profiles[n])+'\n')
        f.close()
    ...
    def Locify(glist): # cluster genes into loci
        locs = {}
        for g in glist:
            l = Encyclopedia.hgnc.loc[g]["location"].split('-')[0]
            if l in locs:
                locs[l].append(g)
            else:
                locs[l] = [g]
    ...
    def Pathify(glist): # cluster genes into pathways
        pth = {}
        for g in glist:
            ps = set([x for x in Encyclopedia.paths if g in Encyclopedia.paths[x]])
            for j in ps:
                if j in pth:
                    pth[j] += 1
                else:
    ...
    # expand operators and brackets
        ...
        for i,w in enumerate(t):
            if w  in '()&|^':  # an operator
                ...
                if w.upper() in Encyclopedia.profiles: # replace with parse of stored profile
                    t[i] = '(' + Functions.prof_flatten(Encyclopedia.profiles[w.upper()]) + ')'
                else:
    ...
    # deal with Universal set    
                ...
                if w[-1] in '-+': # annotated missense GoF / LoF
                    ...
                    if x not in Encyclopedia.hgnc.index:
                        print(f'mutation specification {w} not valid')
                        return ''
                    t[i] = f"Functions.MUT('{x}')"
                else:
                    x = w[1:].upper().split(':')
                    if len(x) != 2 or (x[0] not in Encyclopedia.deps.columns and x[0] not in Encyclopedia.cnvs.columns): # valid gene name ?
                        print(f'mutation specification {w} not valid')
                        return ''
                    t[i] = f"Functions.MUT('{x[0]}')"
            elif w[0] == '@': # load set from file
                t[i] = f"Functions.FIL('{w[1:].upper()}')"    
                
            elif w[0] == '{': # mutational signature
                s = w[1:].upper()
                if s in Encyclopedia.sigs.columns[1:-1]:
                    t[i] = f"Functions.SIG('{s}')"
                else:
                    m = Functions.match_keyword(s,Encyclopedia.SigGroups)
                    if m == '':
                        ...
                        return
                    t[i] = f"Functions.SIG('{m}')"

            elif w[0] == '%': # cancer type selection            
                c = Functions.match_keyword(w[1:].upper(),set(Encyclopedia.info['OncotreeLineage']))
                if c == '':
                ...
                if g in Encyclopedia.deps.columns or g in Encyclopedia.cnvs.columns:

                    if w[0] in '+-': # amplification or deletion
                        t[i] = f"(Functions.AMP('{g}') | Functions.DEL('{g}'))"
                    elif w[0] == '=': # diploid eg wild type copy number
                        t[i] = f"Functions.DIP('{g}')"
                    elif w[0] == '!': # gene disruption
                        t[i] = f"Functions.MUT('{g}')"
                    elif w[0] == '$': # mutationally wild-type
                        t[i] = f"(Functions.WT('{g}') & Functions.DIP('{g}'))"
                    elif w[0] == '~': # fusion
                        t[i] = f"Functions.FUS('{g}')"
                        
                    elif w[0] == '>': # over-expression
                        if w[1] in '0123':
                            s = '0123'.index(w[1])
                            t[i] = f"Functions.OX('{g}',{s})"
                        else:
                            t[i] = f"Functions.OX('{g}',-1)" # -ve signifies use pre-calc thresholds
                    elif w[0] == '<': # under-expression
                        if w[1] in '0123':
                            s = '0123'.index(w[1])
                            t[i] = f"Functions.UX('{g}',{s})"
                        else:
                            t[i] = f"Functions.UX('{g}',-1)" # -ve signifies use pre-calc thresholds             
                                        
                elif gg.split('.')[0] in Encyclopedia.phys_chro.columns:

                    if w[0] == '+': # amplification of locus
                        t[i] = f"Functions.LAMP('{gg}')"
                    elif w[0] == '-': # gene deletion
                        t[i] = f"Functions.LDEL('{gg}')"
                else:
                    print(f'function prefix {f} not valid')
                    return ''
        a = ' '.join(t)
        return f"Functions.UNI()-({a})"
    ...
    # expand operators and brackets
        ...
        for i,w in enumerate(t):
            if w  in '()&|^':  # an operator
                ...
                if w.upper() in Encyclopedia.profiles: # replace with parse of stored profile
                    t[i] = '(' + Functions.prof_parse(Encyclopedia.profiles[w.upper()]) + ')'
                else:
                    print(f'unrecognised token : {w}')
                    return ''
    # deal with Universal set    
                ...
                if w[-1] == '-': # annotated missense LoF
                    ...
                    if x not in Encyclopedia.hgnc.index:
                        print(f'mutation specification {w} not valid')
                        return ''
                    agg = Functions.GeneAggLOF(x)
                    if len(agg) > 0:
                        t[i] = f"Functions.DEM('{x}','{agg}')"
                    else:
                    ...
                    x = w[1:].upper().split(':')
                    if len(x) != 2 or (x[0] not in Encyclopedia.hgnc.index): # valid gene name ?
                        print(f'mutation specification {w} not valid')
                        return ''
                    if x[1][-1] == 'X':
                        x[1][-1] = '?'
    ...
    # 
                if g in Encyclopedia.deps.columns or g in Encyclopedia.cnvs.columns:
                    ...
                    if w[0] == '+': # amplification
                        t[i] = f"Functions.AMP('{g}')"
                    elif w[0] == '-': # gene deletion
                        t[i] = f"Functions.DEL('{g}')"
                    elif w[0] == '>': # over-expression
                        if w[1] in '0123':
                            s = '0123'.index(w[1])
                            t[i] = f"Functions.OX('{g}',{s})"
                        ...
                        t[i] = f"Functions.DIS('{g}')"
                    elif w[0] == '$': # mutationally wild-type
                        t[i] = f"Functions.WT('{g}')"
    ...
    # gene exists but has no mutation or CNV data recorded
                elif g in list(Encyclopedia.hgnc.index) and g not in list(Encyclopedia.muts['HugoSymbol']): 
    ...
    def GetGOF(g=''):
        if g == '':
            a = Encyclopedia.muts.loc[(Encyclopedia.muts['LikelyGoF']) &
                        (Encyclopedia.muts['VariantInfo'] == 'MISSENSE')][['HugoSymbol','ProteinChange']]
            return list(set(zip(a['HugoSymbol'],a['ProteinChange'])))
        else:
            return(list(set(Encyclopedia.muts.loc[(Encyclopedia.muts['LikelyGoF'] ) &
                                    (Encyclopedia.muts['HugoSymbol'] == g) &
                                    (Encyclopedia.muts['VariantInfo'] == 'MISSENSE'),'ProteinChange'])))
            
    def GetLOF(g=''):
        if g == '':
            a = Encyclopedia.muts.loc[(Encyclopedia.muts['LikelyLoF']) &
                        (Encyclopedia.muts['VariantInfo'] == 'MISSENSE')][['HugoSymbol','ProteinChange']]
            return list(set(zip(a['HugoSymbol'],a['ProteinChange'])))
        else:
            return(list(set(Encyclopedia.muts.loc[(Encyclopedia.muts['LikelyLoF'] ) &
                                    (Encyclopedia.muts['HugoSymbol'] == g) &
                                    (Encyclopedia.muts['VariantInfo'] == 'MISSENSE'),'ProteinChange'])))
                                                                                                                                                
    def UNI():
        # return set(set(info['ModelID']) | set(Encyclopedia.muts['ModelID']) | set(Encyclopedia.cnvs['ModelID']) | set(expr['ModelID'])) 
        return set(Encyclopedia.info['ModelID'])
    def AMP(g):
        if g in Encyclopedia.cnvs.columns:
            return set(Encyclopedia.cnvs.loc[(Encyclopedia.cnvs[g] > log2(3.36/2+1)),'ModelID'])
        else :
            return set()    
    def DIP(g):
        if g in Encyclopedia.cnvs.columns:
            return set(Encyclopedia.cnvs.loc[(Encyclopedia.cnvs[g] < log2(2.64/2+1)) &
                            (Encyclopedia.cnvs[g] > log2(1.32/2+1)),'ModelID'])
        else :
            return set()   
    def DEL(g):
        if g in Encyclopedia.cnvs.columns:
            return set(Encyclopedia.cnvs.loc[(Encyclopedia.cnvs[g] < log2(0.87/2+1)),'ModelID'])
        else :
            return set()
    def DIS(g):
        return set(Encyclopedia.muts.loc[(Encyclopedia.muts['HugoSymbol'] == g) & (Encyclopedia.muts['CCLEDeleterious'] | Encyclopedia.muts['LikelyLoF']),'ModelID'])

    def SIG(s): # return set of cells with non-zero occurence of specified signature
        # print(f's = {s}')
        if s in Encyclopedia.sigs.columns[1:-1]:
            return set(Encyclopedia.sigs[(Encyclopedia.sigs[s].div(Encyclopedia.sigs['Burden']) > Encyclopedia.sig_thresh)]['SAMPLES'])
        else:
            rr = set()
            # print(f's is still {s}, SigGroups[s] = {SigGroups[s]}')
                
            for i in Encyclopedia.SigGroups[s]:
                # print(f' i = {i}')
                rr |= set(Encyclopedia.sigs[(Encyclopedia.sigs[i].div(Encyclopedia.sigs['Burden']) > Encyclopedia.sig_thresh)]['SAMPLES'])
            return rr
                
    def WT(g): # anything without a mutation or deletion recorded for gene g or a silent mutation
        return Functions.UNI() ^ (Functions.MUT(g) | Functions.DEL(g) | Functions.AMP(g))
    # def DEML(g,c): # DEfined Mutation with wild card substitions allowed
    #     s2 = []
    #     m = re.compile(r'p#'+c.replace('?','[ACDEFGHIKLMNPQRSTVWY]{1}'))
    #     # print(m)
    #     s1 = Encyclopedia.muts.loc[(Encyclopedia.muts['HugoSymbol'] == g) & (Encyclopedia.muts['VariantInfo'] != 'silent')].reset_index()
    #     for i in s1.index:
    ...
    # substitute, deblank and split mutation list
        ...
        # return set(Encyclopedia.muts.loc[(Encyclopedia.muts['HugoSymbol'] == g) & (Encyclopedia.muts['ProteinChange'] == 'p.'+c),'ModelID'])
    def MUT(g): # any non-silent mutation
        return set(Encyclopedia.muts.loc[(Encyclopedia.muts['HugoSymbol'] == g) & (Encyclopedia.muts['VariantInfo'] != 'SILENT'),'ModelID'])    
    ...
    def TYP(c): # make set from cancer type
        return set(Encyclopedia.info.loc[Encyclopedia.info['OncotreeLineage'] == c]['ModelID'])

    def SEX(s):
        return set(Encyclopedia.info.loc[Encyclopedia.info['Sex'] == s]['ModelID'])

    def AGE(s):
        return set(Encyclopedia.info.loc[Encyclopedia.info['AgeCategory'] == s]['ModelID'])

    def FUS(g): # set of cells in which gene g is involved in a fusion
        r = set(Encyclopedia.fuse[Encyclopedia.fuse['RightGene'] == g]['ModelID'])  
        l = set(Encyclopedia.fuse[Encyclopedia.fuse['LeftGene'] == g]['ModelID'])
        return r | l
    ...
    def LDEL(gg): # make set from match of cnvdels to locus
        ...
        if len(w) > 1:
            ...
            subband = ''
        for c in Encyclopedia.Cdict:
            d = set(Encyclopedia.Cdict[c].cnvdel)
            l = set(Encyclopedia.phys_chro[band]['geneSymbols'])
    ...
    # now test if subband filter is needed
            if subband == '':
                ...
                for g in f:
                    ...
                    if g in Encyclopedia.hgnc.index:
                        gl = Encyclopedia.hgnc.loc[g]['location']
                        # print(f'location {gl}')
    ...
    def LAMP(gg): # make set from match of cnvdels to locus
        ...
        if len(w) > 1:
            ...
            subband = ''
        for c in Encyclopedia.Cdict:
            d = set(Encyclopedia.Cdict[c].cnvamp)
            l = set(Encyclopedia.phys_chro[band]['geneSymbols'])
            f = d & l # main band common set
            # print(f'c {c} f {f}')
    ...
    def OX(g,s):
        if s < 0: # using pre-calculated high/low threshold assigments
            return(set(list(Encyclopedia.expr[Encyclopedia.expr[g] >= Encyclopedia.exval.loc[g]['High']]['ModelID'])))
        if g in Encyclopedia.expr.columns:
            h = Encyclopedia.expr[g].mean()+s*Encyclopedia.expr[g].std()
            return set(Encyclopedia.expr[Encyclopedia.expr[g] >= h]['ModelID'])
        else :
            return set()    

    def UX(g,s):
        if s < 0: # using pre-calculated high/low threshold assigments
            return(set(Encyclopedia.expr[Encyclopedia.expr[g] <= Encyclopedia.exval.loc[g]['Low']]['ModelID']))
        if g in Encyclopedia.expr.columns:
            h = Encyclopedia.expr[g].mean()-s*Encyclopedia.expr[g].std()
            return set(Encyclopedia.expr[Encyclopedia.expr[g] <= h]['ModelID'])
        else :
    ...
    def prof_tree(p,t):  # recursive tree walk 
        for i in Encyclopedia.profiles:
            if p in Encyclopedia.profiles[i].upper().split():
                t.append(i)
                Functions.prof_tree(i,t)
    # =============================================================================
    # 
    ...
    def DandCount(p,q,targ,deps):
        ...
        # q = list(eval(prof_nullhype(Functions.prof_flatten(prof))))
        hype = []
        null = []
        D = 0
    ...
    # get count of p matches above thresh
        ...
        cont[0][0] = sum(deps[deps.index.isin(p)][targ]>Encyclopedia.thresh)
        cont[1][0] = len(p) - cont[0][0]
        
    # get count of q matches above thresh

        cont[0][1] = sum(deps[deps.index.isin(q)][targ]>Encyclopedia.thresh)
    ...
    def DB_save(fgene,fcell):
        ...
        for i in Encyclopedia.Gdict:
            g = Encyclopedia.Gdict[i]
        ...
        for i in Encyclopedia.Cdict:
            c = Encyclopedia.Cdict[i]
    ...
    # should be a gene name so add to dictionary
            Encyclopedia.Gdict[gname] = Gene(gname)
    # get cell line count
            ncell = int(fgene.readline()[:-1].strip())
            for i in range(ncell):
                Encyclopedia.Gdict[gname].cells.append(fgene.readline()[:-1].strip())

            ncnvdel = int(fgene.readline()[:-1].strip())
            for i in range(ncnvdel):
                Encyclopedia.Gdict[gname].cnvdel.append(fgene.readline()[:-1].strip())
            ncnvamp = int(fgene.readline()[:-1].strip())
            for i in range(ncnvamp):
                Encyclopedia.Gdict[gname].cnvamp.append(fgene.readline()[:-1].strip())
                
            nover = int(fgene.readline()[:-1].strip())
            for i in range(nover):
                Encyclopedia.Gdict[gname].over.append(fgene.readline()[:-1].strip())
            nunder = int(fgene.readline()[:-1].strip())
            for i in range(nunder):
                Encyclopedia.Gdict[gname].under.append(fgene.readline()[:-1].strip())
                
        fgene.close()
        print(f'Data for {len(Encyclopedia.Gdict)} genes loaded')
    # do cells
        while True:
            ...
            if cname == '':
                break
    # should be a gene name so add to dictionary
            Encyclopedia.Cdict[cname] = Cell(cname)
            Encyclopedia.Cdict[cname].tissue = fcell.readline()[:-1].strip()
            Encyclopedia.Cdict[cname].cancer = fcell.readline()[:-1].strip()

    # get damaged genes count
            ngene = int(fcell.readline()[:-1].strip())
            for i in range(ngene):
                Encyclopedia.Cdict[cname].dgenes.append(fcell.readline()[:-1].strip())
    # get mutated genes count
            ngene = int(fcell.readline()[:-1].strip())
            for i in range(ngene):
                Encyclopedia.Cdict[cname].mgenes.append(fcell.readline()[:-1].strip())
    # get CNV deleted count
            ngene = int(fcell.readline()[:-1].strip())
            for i in range(ngene):
                Encyclopedia.Cdict[cname].cnvdel.append(fcell.readline()[:-1].strip())
    # get CNV amplified count
            ngene = int(fcell.readline()[:-1].strip())
            for i in range(ngene):
                Encyclopedia.Cdict[cname].cnvamp.append(fcell.readline()[:-1].strip())
    # get overexpression count
            nover = int(fcell.readline()[:-1].strip())
            for i in range(nover):
                Encyclopedia.Cdict[cname].over.append(fcell.readline()[:-1].strip())
    # get underexpression count
            nunder = int(fcell.readline()[:-1].strip())
            for i in range(nunder):
                Encyclopedia.Cdict[cname].under.append(fcell.readline()[:-1].strip())
            
                
        fcell.close()
        print(f'Data for {len(Encyclopedia.Cdict)} cells loaded')       
    ...
    def UserChoiceFilesAndParam():
        a = input('Specify files and parameters ? ([N]/Y] : ').upper()
        if len(a) > 0 and a[0] == 'Y':
            Encyclopedia.spec = True

    def FileLoadIn(defaultfile,filenames):
        defaultfile = filenames
        Encyclopedia.f = defaultfile
        f = Encyclopedia.f
# LHP
        # if Encyclopedia.spec:
        #     f = input(f'Gene dependency file [{defaultfile}] : ')
        #     if f.strip() == '' :
        #         f = defaultfile
        print(f'Loading {f}')
        return(f)             
...
# load dependency data - 'deps'

        f = Encyclopedia.default_dep_dat_file
        print(f'Loading {f}')
        
        Encyclopedia.deps = pd.read_csv(f).fillna(0.0)
        Encyclopedia.deps_loaded = True
        
        # make cell line index to dependency data
        
        Encyclopedia.deps.set_index('ModelID', inplace = True)
        
        # edit header names to conform to remove extraneous data
        
        gg = dict(zip(list(Encyclopedia.deps.columns),[i.split()[0] for i in list(Encyclopedia.deps.columns)]))
        Encyclopedia.deps.rename(columns = gg, inplace = True)
        
# load model information - 'info'

        f = Encyclopedia.default_cell_info_file
        print(f'Loading {f}')
        
        Encyclopedia.info = pd.read_csv(f, low_memory=False).fillna('')
        Encyclopedia.info_loaded = True
        Encyclopedia.info['OncotreeLineage'] = [x.upper() for x in Encyclopedia.info['OncotreeLineage']] # make cancer types upper case
        cancer_types = set(Encyclopedia.info['OncotreeLineage'])
        
# load mutation data - 'muts'

        f = Encyclopedia.default_mut_dat_file
        print(f'Loading {f}')

        Encyclopedia.muts = pd.read_csv(f, low_memory=False).fillna('')   
        Encyclopedia.muts_loaded = True

# load physical chromosome data - 'phys_chro'

        f = Encyclopedia.default_phys_chro_file
        print(f'Loading {f}')

        Encyclopedia.phys_chro = pd.read_json(f).fillna('')   
        Encyclopedia.phys_chro_loaded = True

# load HUGO gene reference data - 'hgnc'

        f = Encyclopedia.default_HUGO_file
        print(f'Loading {f}')
        
        Encyclopedia.hgnc = pd.read_table(f, low_memory=False).fillna('')
        Encyclopedia.hgnc = Encyclopedia.hgnc[['symbol','ensembl_gene_id','prev_symbol','location','location_sortable']]
        Encyclopedia.hgnc.set_index('symbol',inplace = True)
        Encyclopedia.hgnc_loaded = True

    # correct legacy Encyclopedia.deps gene symbols to HUGO

        bad_names = set(Encyclopedia.deps.columns) & (set(Encyclopedia.hgnc.index) ^ set(Encyclopedia.deps.columns)) 
        print(f'Updating {len(bad_names)} archaic gene names in dependency data')
        for g in bad_names:
            g2 = Encyclopedia.hgnc[Encyclopedia.hgnc['prev_symbol'].str.contains(g)].reset_index()['symbol']
            if len(g2) == 0 or (g2[0] not in Encyclopedia.hgnc.index):
                # print(f'DepMap Gene name {g} not found in HUGO - ignoring it')
                continue
            else:
                # print(f'DepMap old gene name {g} replaced by new name {g2[0]}')
                Encyclopedia.deps.rename(columns={g:g2[0]}, inplace=True)
                
# load copy number variation data - 'cnvs'

        f = Encyclopedia.default_cnv_dat_file
        print(f'Loading {f}')

        Encyclopedia.cnvs = pd.read_csv(f, low_memory=False).fillna(0.0)
        Encyclopedia.cnvs_loaded = True

        Encyclopedia.default_cnv_dat_file = f
        print(f'Loading {f}')

        # make cell line index to CNV data

        Encyclopedia.cnvs.rename( columns={'Unnamed: 0':'ModelID'}, inplace=True )
        Encyclopedia.cnvs.set_index('ModelID', inplace = True)

        # edit header names to conform to remove extraneous data

        gg = dict(zip(list(Encyclopedia.cnvs.columns),[i.split()[0] for i in list(Encyclopedia.cnvs.columns)]))
        Encyclopedia.cnvs.rename(columns = gg, inplace = True)
        
# load gene fusion data - 'fuse'

        f = Encyclopedia.default_fus_dat_file
        print(f'Loading {f}')

        Encyclopedia.fuse = pd.read_csv(f, low_memory=False).fillna(0.0)
        Encyclopedia.fuse_loaded = True
        Encyclopedia.fuse['LeftGene'] = [x.split()[0] for x in list(Encyclopedia.fuse['LeftGene'])]
        Encyclopedia.fuse['RightGene'] = [x.split()[0] for x in list(Encyclopedia.fuse['RightGene'])]
        
# load gene expression data - 'expr'

        f = Encyclopedia.default_expr_dat_file
        print(f'Loading {f}')

        Encyclopedia.expr = pd.read_csv(f, low_memory=False).fillna(0.0)
        Encyclopedia.expr_loaded = True
        
        # make cell line index to expression data

        Encyclopedia.expr.rename( columns={'Unnamed: 0':'ModelID'}, inplace=True )
        Encyclopedia.expr.set_index('ModelID', inplace = True)

        # edit header names to conform to remove extraneous data

        gg = dict(zip(list(Encyclopedia.expr.columns),[i.split()[0] for i in list(Encyclopedia.expr.columns)]))
        Encyclopedia.expr.rename(columns = gg, inplace = True)

        
# load gene expression threshold value data - 'exval'

        f = Encyclopedia.default_expr_thresh_file
        print(f'Loading {f}')
        
        Encyclopedia.exval = pd.read_csv(f)
        Encyclopedia.exval.set_index('HugoSymbol', inplace = True)
        Encyclopedia.exval_loaded = True

# load mutational signatire data - 'exval'

        f = Encyclopedia.default_signatures_file
        print(f'Loading {f}')
        
        Encyclopedia.sigs = pd.read_table(f, low_memory=False).fillna('')
        gg = dict(zip(list(Encyclopedia.sigs.columns),[i.upper() for i in list(Encyclopedia.sigs.columns)]))
        Encyclopedia.sigs.rename(columns = gg, inplace = True)

        # # make mutational burden column
        Encyclopedia.sigs['Burden'] = Encyclopedia.sigs.sum(axis=1, numeric_only=True)

# load functional pathways data - 'exval'

        f = Encyclopedia.default_pathways_file
        print(f'Loading {f}')
        ...
        while True:
            ...
            if line == '':
                break
            w = line.split()
            Encyclopedia.paths[w[0]] = set(w[2:])
            
# load chromosome length data - 'chrom'

        f = Encyclopedia.default_chromolen_file
        print(f'Loading {f}')

        Encyclopedia.chrom = pd.read_csv(f)
        Encyclopedia.chromlen_loaded = True
        
# load ENSEMBLE - 'ensem'

        f = Encyclopedia.default_ENSEMBLE_file       
        print(f'Loading {f}')
        
        Encyclopedia.ensem = pd.read_table(f)
        Encyclopedia.ensem.drop_duplicates(subset = ['gene_stable_id'],inplace = True)
        Encyclopedia.ensem = Encyclopedia.ensem[['gene_stable_id','transcript_stable_id']]
        Encyclopedia.ensem.set_index('gene_stable_id',inplace = True)
        Encyclopedia.ensem_loaded = True

            
# load DepMine local Gene and Cell Databases 

        print('Loading DepMine Databases')

        f_gene = open(Encyclopedia.default_GeneDatabase_file,'r')
        f_cell = open(Encyclopedia.default_CellDatabase_file,'r')
        Functions.DB_load(f_gene,f_cell)
        
        
    # reset index struture of Encyclopedia.cnvs
        Encyclopedia.cnvs.reset_index(inplace = True)
    # reset index struture of Encyclopedia.expr
        Encyclopedia.expr.reset_index(inplace = True)
...
# =============================================================================
# Load/Save Encyclopedia.profiles
# =============================================================================
...
# primary cancer type factors - may become free standing later
        ...
        if task.lower() == 'prime':
            dis = Functions.GetOncotreePrimaryDisease()
            
# loop through diseases            

            pool = mp.Pool(mp.cpu_count())
            results = pool.starmap_async(Functions.MakeGoFFing,
                        [(d,Encyclopedia.info,Encyclopedia.muts) for d in dis]).get()

                # results = pool.starmap_async(ProfPairPvalue,
                #         [(g,prof,p,q,Encyclopedia.deps,thresh) for g in Encyclopedia.deps.columns]).get()
            pool.close()
...
#                 pGoFs = Encyclopedia.muts[Encyclopedia.muts['ModelID'].isin(p) & Encyclopedia.muts['LikelyGoF'] == True] \
#                             [['HugoSymbol','ProteinChange']]
#                 lg = list(pGoFs['HugoSymbol'])
#                 lc = list(pGoFs['ProteinChange'])
# # combine into mutation
...
#                 qGoFs = Encyclopedia.muts[Encyclopedia.muts['ModelID'].isin(q) & Encyclopedia.muts['LikelyGoF'] == True] \
#                             [['HugoSymbol','ProteinChange']]
#                 lg = list(qGoFs['HugoSymbol'])
#                 lc = list(qGoFs['ProteinChange'])
# # combine into mutation
...
# load HUGO chromosomal location data
            f = Encyclopedia.default_HUGO_file = 'hgnc_complete_set.tsv'
            if Encyclopedia.spec:
                f = input(f'HUGO Gene file [{Encyclopedia.default_HUGO_file}] : ')
                if f.strip() == '' :
                    f = Encyclopedia.default_HUGO_file
...
# load WRN GeneB hit list
            ...
            if not Encyclopedia.info_loaded:
                Encyclopedia.default_cell_info_file = 'Model.csv'
                f = input(f'Cell sample information file [{Encyclopedia.default_cell_info_file}] : ')
                if f.strip() == '' :
                    f = Encyclopedia.default_cell_info_file
                print(f'Loading {f}')
            
                Encyclopedia.info = pd.read_csv(f)
                Encyclopedia.info_loaded = True
                Encyclopedia.info['OncotreeLineage'] = [x.upper() for x in Encyclopedia.info['OncotreeLineage']] # make cancer types upper case
                cancer_types = set(Encyclopedia.info['OncotreeLineage'])
            ...
            Encyclopedia.seaborn_plot = not Encyclopedia.seaborn_plot
...
# =============================================================================
            ...
            Encyclopedia.first_call = True
            while not Encyclopedia.prof_loaded:
                Encyclopedia.profiles = {}
                Functions.list_profile_files()
                default_prof_file = 'AB_Dep_Profiles'
                f = input(f'Default Profile file [{default_prof_file}] : ')
                if f.strip() == '' :
                    f = default_prof_file
                print(f'Loading {f}')
                if Functions.prof_load(f):
                    print(f'{f} loaded')
                    Encyclopedia.prof_loaded = True
                    break
                else:
                    continue
            if len(Encyclopedia.profiles) < 1:
...
# get genetic profile to use
            ...
            while True:
                if Encyclopedia.first_call and len(Encyclopedia.profiles) > 1:
                    print('\nAvailable predefined genetic profiles :')
                    for n in sorted(Encyclopedia.profiles):
                        print(n)
                    Encyclopedia.first_call = False
...
# check that profile exists
                m = Functions.match_keyword(gp.upper(),Encyclopedia.profiles)
                if  m == '*':
                    print(f'{gp.upper()} is ambiguous')
                    continue
                if m not in Encyclopedia.profiles:
                    print(f'Genetic profile : {gp} is not defined\nDo this in LINE\n')
                    continue

                prof = Encyclopedia.profiles[m]
                
                p = list(eval(Functions.prof_parse(prof)))
                
                if len(p) > 0:
                    Functions.SigType(p,m)
                else:
...
# generate null set
                q = list(eval(Functions.prof_nullhype(Functions.prof_flatten(prof))))
                ...
                if len(ans) == 0 or ans[0].upper() == 'Y':
                    Functions.PlotCellType(p)

                print(f'Calculating dependency for all targets in {len(p)} cell lines with genetic profile {m}')


# create Encyclopedia.deps subset from p

                depsub = Encyclopedia.deps[Encyclopedia.deps.index.isin(list(p))]
                dmeans = {}
                
                for g in depsub.columns:
                    d = depsub[g].mean()
                    if d >Encyclopedia.thresh:
                        dmeans[g] = d
                
                               
                high_deps = dict(sorted(dmeans.items(),key = lambda x: x[1], reverse = True))

                print(f'Cells matching genetic profile {m} have {len(high_deps)} genes with |dep| > {Encyclopedia.thresh} ')
...
# look for known oncogenes in this set
                ...
                if not Encyclopedia.oncogenes_loaded:
                    ...
                    Encyclopedia.oncogenes = pd.read_csv(f)
                    Encyclopedia.oncogenes_loaded = True

# look for known GOFs in filtered list
                
                high_genes['Oncogene'] = high_genes['HugoSymbol'].isin(Encyclopedia.oncogenes['HugoSymbol'])  
                

                pgof = set([x[0] for x in sorted(Functions.GetGOF())])
...
# =============================================================================
            ...
            if a != '' and a[0] == 'E':
                Encyclopedia.gofs = pd.read_csv('Final_GOF_all_table.csv')
                gof = [(Encyclopedia.gofs.iloc[i]['Gene_name'],'p.'+Encyclopedia.gofs.iloc[i]['Substitution']) for i in Encyclopedia.gofs.index]
            
# get GOF list            
            else:
                gof = sorted(Functions.GetGOF())
...
# loop through GOFs - for individual or residue aggregated
    ...
    # now parallelise over all genes in Encyclopedia.deps
    
                    results = []
                    t_base = timer()
    
                    pool = mp.Pool(mp.cpu_count())
    #                glist = list(set(Encyclopedia.deps.columns) | set(Encyclopedia.cnvs.columns[1:]))
                    glist = list(set(Encyclopedia.deps.columns))
    
                    qs = random.choices(q,k=int(min(len(q)/2,10*len(p)))) # set null set sample size
                    print(f'null set using sample size = {len(qs)}')
                    results = pool.starmap_async(Functions.AltProfPairPvalue,
                            [(g,prof,p,qs,Encyclopedia.deps,Encyclopedia.thresh,Encyclopedia.null_trim) for g in glist]).get()
    ...
    # sort on p values
                    ...
                    for i in Plist:
                        if Plist[i] < Encyclopedia.p_limit: # hypethesis is True
                            HitList[i] = Plist[i]
                    
                    if len(HitList) > 0 :
                        print(f'{len(HitList)} dependency switches found for genetic profile {prof} with p<={Encyclopedia.p_limit}\n')                     
                            
                for h in HitList:
                    p = eval(Functions.prof_parse(prof))
                    q = eval(Functions.prof_nullhype(Functions.prof_flatten(prof)))
                    match,notmatch = Functions.ProfPairDist(h,p,q,Encyclopedia.deps,Encyclopedia.null_trim)                   
    # calculate Cohen's D
                    D = (np.array(match).mean()-np.array(notmatch).mean())/np.array(match+notmatch).std()
                    if abs(D)> Encyclopedia.cohen_min:
...
#                glist = list(set(Encyclopedia.deps.columns) | set(Encyclopedia.cnvs.columns[1:]))
                ...
                print(f'null set using sample size = {len(qs)}')
                results = pool.starmap_async(Functions.AltProfPairPD,
                        [(g,list(p),qs,Encyclopedia.deps,Encyclopedia.thresh) for g in glist]).get()
                
# =============================================================================
#                 results = pool.starmap_async(ProfPairPvalue,
#                         [(g,prof,p,q,Encyclopedia.deps,thresh) for g in glist]).get()
# 
# =============================================================================
                # results = pool.starmap_async(ProfPairPvalue,
                #         [(g,prof,p,q,Encyclopedia.deps,thresh) for g in Encyclopedia.deps.columns]).get()
...
# sort on p values
                ...
                for i in Plist:
                    if Plist[i] < Encyclopedia.p_limit and Dlist[i] > Encyclopedia.cohen_min: # hypothesis is True
                        HitList[i] = Plist[i]

                if len(HitList) > 0 :
                    print(f'{len(HitList)} dependency switches found for aggregated GOF for gene {gg} with p<={Encyclopedia.p_limit:.2} and D>{Encyclopedia.cohen_min:.2} \n')                     
                    for h in HitList:
            ...
            Encyclopedia.gofs = pd.read_table(f)
            Encyclopedia.gofs.sort_values(by = 'p', inplace = True)
            Encyclopedia.gofs.reset_index(inplace = True)
            del Encyclopedia.gofs['index']
            
            print(Encyclopedia.gofs)
# loop through gofs calculating D
            for i in Encyclopedia.gofs.index:
                gB = Encyclopedia.gofs.iloc[i]['GOF']
                gA = Encyclopedia.gofs.iloc[i]['GeneA']
                pval = Encyclopedia.gofs.iloc[i]['p']
                D = Encyclopedia.gofs.iloc[i]['D']
                if abs(D) > Encyclopedia.cohen_min:
...
# trim gB name if gene aggregated
                    ...
                    p = eval(Functions.prof_parse(gB))
                    q = eval(Functions.prof_nullhype(Functions.prof_flatten(gB)))
                    match,notmatch = Functions.ProfPairDist(gA,p,q,Encyclopedia.deps,Encyclopedia.null_trim)                   
...
# generate hypothesis set - cleaned up by having Encyclopedia.deps data
                p = list(eval(Functions.prof_parse(prof)) & set(Encyclopedia.deps.index))
# generate null set - cleaned up by having Encyclopedia.deps data
                q = list(eval(Functions.prof_nullhype(Functions.prof_flatten(prof))) & set(Encyclopedia.deps.index))
                print(f'{len(p)} cell lines in matching profile set')
                print(f'{len(q)} cell lines in null set')
# check for null sets
...
#                glist = list(set(Encyclopedia.deps.columns) | set(Encyclopedia.cnvs.columns[1:]))
                ...
                results = pool.starmap_async(Functions.AltProfPairPvalue,
                        [(g,prof,p,q,Encyclopedia.deps,Encyclopedia.thresh,Encyclopedia.null_trim) for g in glist]).get()
...
# make boxplots for HitList targets        
                ...
                plt.title(f'Switching targets for {m} with p < {Encyclopedia.p_limit}',fontsize = '20',pad=20)
...
# ProfPairPvalue(target[0],prof,p,q,Encyclopedia.deps,thresh)
            ...
            while True:
                ...
                if len(a) == 0 or a[0] == 'Q':
                    ...
                    if len(add) > 0 and add[0] == 'R':
                        Encyclopedia.profiles = {}
                        Encyclopedia.prof_loaded = False
                    Functions.list_profile_files()
                    default_prof_file = 'AB_Dep_Profiles'
                    f = input(f'Select profile file [{default_prof_file}] : ')
                    if f.strip() == '' :
                    ...
                    print(f'Loading {f}')
                    if Functions.prof_load(f):
                        print(f'{len(Encyclopedia.profiles)} loaded from {f}')
                        Encyclopedia.prof_loaded = True
                    else:
                        continue
                    ...
                    if len(add) > 0 and add[0] == 'O':
                        Functions.prof_save(def_prof_file)
                        print(f'{len(Encyclopedia.profiles)} profiles written to default file {def_prof_file}')
                        continue
                    while True:
                        ...
                        if len(b) > 0:
                            ...
                            if pname in glob('*rofiles'): # danger of overwriting
                                ...
                                if len(b) > 0 and b[0] != 'O':
                                    break
                            Functions.prof_save(pname)    
                            print(f'{len(Encyclopedia.profiles)} profiles written to {pname}')
                            break
                    ...
                    m = Functions.match_keyword(w[1],Encyclopedia.profiles)
                    if  m == '*':
                        print(f'{w[1]} is ambiguous')
                    elif m in Encyclopedia.profiles:
                        print(f'\n{m} : {Functions.prof_flatten(Encyclopedia.profiles[m])}')
                    else:
...
# build list of profiles that depend on specified profile
                        p=[]                    
                        Functions.prof_tree(m,p)
                        if len(p) > 0:
                            print(f'\nDeleting {m} will also remove {Functions.Flistand(p)}')
                        b = input(f'Are you sure you want to delete {m} ? (Y/[N] : ')
                        if len(b) > 0 and b[0].lower() == 'y':
                            del Encyclopedia.profiles[m]
                            for i in p:
                                del Encyclopedia.profiles[i]
                                
                            add = input('Create new profile file, Overwrite default, Quit ([C],O,Q) : ').upper()
                            if len(add) > 0 and add[0] == 'Q':
                                continue
                            if len(add) > 0 and add[0] == 'O':
                                Functions.prof_save('AB_Dep_Profiles')
                                print(f'{len(Encyclopedia.profiles)} profiles written to default file')
                                continue
                            while True:
                                b = input('New profile file name ? (xxxx.profiles) : ' ).strip()
                                if '.profiles' in b:
                    ...
                    if m not in Encyclopedia.profiles:
                        print(f'{w[1]} is not a known profile')
                        continue
                    if w[2] in Encyclopedia.profiles:
                        print(f'{w[2]} is already a profile name')
                        continue
                    for i in Encyclopedia.profiles:
                        if m in Encyclopedia.profiles[i].upper():
                            print(f'Updating reference to {m} in {i} to {w[2]}')
                            Encyclopedia.profiles[i] = Encyclopedia.profiles[i].replace(m.lower(),w[2].lower())
                    Encyclopedia.profiles[w[2]] = Encyclopedia.profiles[m]
                    del Encyclopedia.profiles[m]
...
# =============================================================================
            ...
            while True:
                if Encyclopedia.first_call and len(Encyclopedia.profiles) > 0:
...
# loop through cell lines acquiring those that match condition string
                ...
                if len(hits) > 0:
                    ...
                    if len(a) > 0 and a.lower()[0] != 'n':
                        ...
                        fl.close()
                        for h in hits:
                            f = Encyclopedia.info.loc[Encyclopedia.info['ModelID'] == h]
                            if len(f) > 0:
...
# check if name already exists in profiles
                            if u_name in Encyclopedia.profiles:
...
# add to profiles
                            Encyclopedia.profiles[u_name] = s
                            Functions.prof_save('TEMP_Profiles')
...
# load chromosome length data
            ...
            if not Encyclopedia.chromlen_loaded:    
                default_chromolen_file = 'ChromosomeLengths.csv'
...
# load Ensemble refseq gene-to-transcript mapping file
            ...
            if not Encyclopedia.ensem_loaded:
                default_ENSEMBLE_file = 'Homo_sapiens.GRCh38.110.refseq.tsv'
...
# try and match as ModelID
                if c not in list(Encyclopedia.info['ModelID']): # not model ID so try CellLineName
                    cn = Encyclopedia.info[Encyclopedia.info['StrippedCellLineName'].str.contains(c)]['ModelID']        
                    if len(cn) < 1:
                    ...
                    if len(cn) > 1:
                        ...
                        print('Choose from :')
                        for i in cn:
                            clong = list(Encyclopedia.info[Encyclopedia.info['ModelID'] == i]['StrippedCellLineName'])[0]
                            print(f'{i} - {clong}')
                        continue
                    c = cn.iloc[0]
                    clong = list(Encyclopedia.info[Encyclopedia.info['ModelID'] == c]['StrippedCellLineName'])[0]
# draw chromosomes
...
# loop through amplified and deleted genes
                    ...
                    for t in Encyclopedia.Cdict[c].cnvamp:
                        d = Functions.ChroLoc(t,Encyclopedia.hgnc,Encyclopedia.ensem)
                        if d == None:
...
# add lines for genes                                
                    ...
                    for t in Encyclopedia.Cdict[c].cnvdel:
                        d = Functions.ChroLoc(t,Encyclopedia.hgnc,Encyclopedia.ensem)
                        if d == None:
                            print(f'Failed to get location data for gene : {t}')
                            continue
...
#  check that first parameter is valid gene name
                ...
                if w[0].upper() in Encyclopedia.hgnc.index:
                    geneA = w[0].upper()
                else:
                ...
                if '.' in w[1] and w[1] in set(Encyclopedia.hgnc['location']): # is it a sub-band
                    gloc = list(Encyclopedia.hgnc[Encyclopedia.hgnc['location'] == w[1]].index)    
                    
                else: # major band
                    l1 = pd.DataFrame(list(Encyclopedia.hgnc.index),[x.split('.')[0] for x in Encyclopedia.hgnc['location']]).reset_index()
...
# now loop through gene list calculating p and d but only for LoF mutations
                ...
                for g in gloc:
                    p = set()
                    pf1 = Functions.prof_parse(f'!{g}')
                    if pf1 != '':
                        p = eval(pf1)
                    pf2 = Functions.prof_parse(f'/{g}-')
                    if pf2 != '':
                        p |= eval(pf2)
                    ...
                    q = list(eval(Functions.prof_parse(f'${g}')))
                        
                    r = Functions.AltProfPairPD(geneA,p,q,Encyclopedia.deps,Encyclopedia.thresh) # calc p and d
                    pvals[g] = r[1]
                    dvals[g] = r[2]
                    ps[g] = p
                    qs[g] = q
...
# plot waterfall plot of effect size
                ...
                for n,i in enumerate(dvals):
                    if pvals[i] < Encyclopedia.p_limit:
                ...
                for g in pvals:
                    if pvals[g] < Encyclopedia.p_limit:
                        ...
                        ax.tick_params(axis='both', labelsize=10)
                        plt.ylim(0.0,1.0)
                        match,notmatch = Functions.ProfPairDist(geneA,ps[g],qs[g],Encyclopedia.deps,Encyclopedia.null_trim)                   
...
# load HUGO chromosomal location data
            ...
            Encyclopedia.default_HUGO_file = 'hgnc_complete_set.tsv'
            f = input(f'HUGO Gene file [{Encyclopedia.default_HUGO_file}] : ')
            if f.strip() == '' :
                f = Encyclopedia.default_HUGO_file
            print(f'Loading {f}')
            Encyclopedia.hgnc = pd.read_table(f)
            Encyclopedia.hgnc = Encyclopedia.hgnc[['symbol','ensembl_gene_id','prev_symbol','location','location_sortable']]
            Encyclopedia.hgnc.set_index('symbol',inplace = True)
...
# =============================================================================
            Encyclopedia.default_dep_dat_file = 'AB_Dep_AllByAll_pairs'
            
            f = input(f'Gene AB pair file [{Encyclopedia.default_dep_dat_file}] : ')
            if f.strip() == '' :
                f = Encyclopedia.default_dep_dat_file
            print(f'Loading {f}')
...
# =============================================================================
            ...
            while True:
                ...
                if usemuts or usecnv or useexp:
                    print(f'Using {Functions.Flistand(s)} ')
                    break
                else:
            ...
            while True:
                ...
                if t != '' and t[0] =='q' :
                    ...
                    while True:
                        ...
                        gB = pairs['GeneB'].iloc[i]
                        pAB = pairs['p'].iloc[i]
                        mut,notmut = Functions.PairDist(gA,gB,Encyclopedia.Cdict,Encyclopedia.deps,usemuts,usecnv,useexp)
...
# load HUGO chromosomal location data
                    ...
                    if not Encyclopedia.hgnc_loaded:
                        Encyclopedia.default_HUGO_file = 'hgnc_complete_set.tsv'
                        f = input(f'HUGO Gene file [{Encyclopedia.default_HUGO_file}] : ')
                        if f.strip() == '' :
                            f = Encyclopedia.default_HUGO_file
                        print(f'Loading {f}')
                        Encyclopedia.hgnc = pd.read_table(f)
                        Encyclopedia.hgnc = Encyclopedia.hgnc[['symbol','ensembl_gene_id','location','location_sortable','prev_symbol']]
                        Encyclopedia.hgnc.set_index('symbol',inplace = True)
                        Encyclopedia.hgnc_loaded = True
...
# obtain Ensemble transcript from HUGO name
                                ...
                                if hgid in Encyclopedia.hgnc.index:
                                    egid = Encyclopedia.hgnc.loc[hgid]['ensembl_gene_id']
                                elif hgid in list(Encyclopedia.hgnc['prev_symbol']):
                                    egid = Encyclopedia.hgnc.iloc[Encyclopedia.hgnc['prev_symbol'].tolist().index(hgid)]['ensembl_gene_id']
                                else:
                                    print(f'GeneB : {hgid} not found in HUGO symbol lists - ignored')
                                    continue
                                # print(f'HUGO : {hgid} = ENSG : {egid}')
                                if egid in Encyclopedia.ensem.index:
                                    etid = Encyclopedia.ensem.loc[egid]['transcript_stable_id']
                                else:
                                    print(f'GeneA : {egid} for HUGO symbol {hgid} not found in ENSEMBLE transcripts - ignored')
                                    continue
                                # print(f'ENSG: {egid} = ENST : {etid}')
# use Ensemble API to get data                                
                                ...
                                print(f"{hgid} - {Encyclopedia.hgnc.loc[hgid]['location']}")

# get chromosomal info for GeneA as well

                        if gA in Encyclopedia.hgnc.index:
                            egid = Encyclopedia.hgnc.loc[gA]['ensembl_gene_id']
                        elif gA in list(Encyclopedia.hgnc['prev_symbol']):
                            egid = Encyclopedia.hgnc.iloc[Encyclopedia.hgnc['prev_symbol'].tolist().index(gA)]['ensembl_gene_id']
                        else:
                            print(f'GeneB : {hgid} not found in HUGO symbol lists - ignored')
                            continue
                        # print(f'HUGO : {hgid} = ENSG : {egid}')
...
# =============================================================================
        ...
        elif task.lower() == 'Encyclopedia.deps' :
...
# check that target is valid gene name
                if target[0] not in Encyclopedia.deps.columns:
...
# check that profile(s) exists
                m = Functions.match_keyword(target[1],Encyclopedia.profiles)
                if  m == '*':
                ...
                prof = Encyclopedia.profiles[target[1]]
...
# get dependency on target[0]
                    ...
                    if h in Encyclopedia.deps.index:
                        d = Encyclopedia.deps[target[0]].loc[h]
...
# check that profile(s) exists
                ...
                if len(target) == 3:
                    m = Functions.match_keyword(target[2],Encyclopedia.profiles)
                    if  m == '*':
...
# if only one profile given generate null set
                if len(target) == 2:
                    q = list(eval(Functions.prof_nullhype(Functions.prof_flatten(prof))))
                    pd = set(p) & set(Encyclopedia.deps.index)
                    qd = set(q) & set(Encyclopedia.deps.index)
                    ...
                    prof2 = Encyclopedia.profiles[target[2]]
                    q = list(eval(Functions.prof_parse(prof2)))
# check for null sets
                ...
                match,notmatch = Functions.ProfPairDist(target[0],p,q,Encyclopedia.deps,Encyclopedia.null_trim)
                r = Functions.ProfPairPvalue(target[0],prof,p,q,Encyclopedia.deps,Encyclopedia.thresh,Encyclopedia.null_trim) # returns [target name,p]
                pAB = r[1]
...
# calculate Cohen's D
                ...
                if len(target) == 2:
                    Functions.DepPlot2(match,notmatch,target[0],target[1].replace('_','-'),pAB,D)
                elif len(target) == 3:
                    Functions.DepPlot2(match,notmatch,target[0],target[1].replace('_','-'),pAB,D,l1=target[1].replace('_','-'),l2=target[2].replace('_','-'))

# initialise factor list

                factors = []
                froot = '('+Functions.prof_flatten(prof)+')'
                nfact = 0
                d1 = Encyclopedia.deps[target[0]].reset_index()
                    ...
                    for c in sex:
                        ...
                        with np.errstate(divide = 'ignore', invalid = 'ignore'):
                            ...
                            if len(dmut) > 0 and len(dwt) > 0:
                                ...
                                Dd = 0
                        # print(f'p = {Pval}')
                        if Pval < 0.05 and abs(Dd) >= Encyclopedia.cohen_min:
                            cD[c] = Dd
...
# encode additional factor
                            ...
                            gdeps,nomatch = Functions.ProfPairDist(target[0],pdis,qdis,Encyclopedia.deps,Encyclopedia.null_trim) # get Encyclopedia.deps for these cells
...
# get subset of hypothesis set that have this cancer type
                ...
                # Encyclopedia.sig_thresh = 0.1
                if t != '' and t[0].upper() == 'Y':
                    f = input(f'Minimum fraction of cell mutational burden to consider a signature [{Encyclopedia.sig_thresh}] : ')
                    if f.strip() != '' :
                        try:
                            Encyclopedia.sig_thresh = float(f)
                        except ValueError:
                            print(f'Invalid value : using {Encyclopedia.sig_thresh}')
                    if Encyclopedia.sig_thresh < 0.0 or Encyclopedia.sig_thresh > 1.0:
                        Encyclopedia.sig_thresh = 0.05
                        print(f'Invalid value : resetting {Encyclopedia.sig_thresh}')

# get signature type count above threshold for the hypothesis set
                    signatures = {}
                    psigs = Encyclopedia.sigs.loc[Encyclopedia.sigs['SAMPLES'].isin(p)] # data for cells in p
                    pd.set_option('mode.chained_assignment', None)
                    psigs[psigs.columns[1:-1]] = psigs[psigs.columns[1:-1]].div(Encyclopedia.sigs['Burden'],axis = 0)
                    pd.set_option('mode.chained_assignment', 'warn')
                    print(f'{len(psigs)} cell lines included')
                    
                    for s in psigs.columns[1:-1]:
                        sl = list(psigs[psigs[s] > Encyclopedia.sig_thresh]['SAMPLES'])
                        # print(s,sl)
                        if len(sl) > 0:
                            signatures[s] = len(sl)
...
# get subset of hypothesis set that have this signature type
                    ...
                    if len(csig) == 0:
                        ...
                        for c in csig:
                            ...
                            if csig[c] < 0:
                                ...
                                tt = 'high'
                            print(f'Mutational signature {c} ({Functions.WhichSigType(c)}) is highly significantly over-represented - p = {abs(csig[c])} d = {cD[c]}\n',
                                  f'in profile matching cells with {tt} dependency on {target[0]}')
                            t = input('Plot affected cells ([Y],N,QUIT) : ')
                ...
                Encyclopedia.frac_limit = 0.1    
                t = input('Analyse disruptive / LoF missense mutational distribution (Y,[N]) : ')
                if t != '' and t[0].upper() == 'Y':
                    f = input(f'Minimum fraction of cells to consider a gene effect [{Encyclopedia.frac_limit}] : ')
                    if f.strip() != '' :
                        try:
                            Encyclopedia.frac_limit = float(f)
                        except ValueError:
                            print(f'Invalid value : using {Encyclopedia.frac_limit}')
                    if Encyclopedia.frac_limit < 0.0 or Encyclopedia.frac_limit > 1.0:
                        Encyclopedia.frac_limit = 0.05
                        print(f'Invalid value : resetting {Encyclopedia.frac_limit}')
                    
                    sp = set(p)
                    sq = set(q)
                    n_limit = len(sp)*Encyclopedia.frac_limit
                    
# gather all disruptive mutations in profile matching cells
                    mall = []                    
                    for c in p:
                        if c in Encyclopedia.Cdict:
                            mall += Encyclopedia.Cdict[c].dgenes
                    muniq = set(mall)
                    ...
                    print(f'{len(gcount)} genes are mutated in > {(Encyclopedia.frac_limit)*100}% of the matching cell lines')
# go through genes calcualting t-test
                    ...
                    for g in gcount:
                        ...
                        cmut = (Functions.DIS(g) | Functions.DEM(g,Functions.GeneAggLOF(g))) & sp # cells in matched set with m gene disruption
                        if cmut == set():
                    ...
                    if len(gsig) == 0:
                        ...
                        for g in gsig:
                            ...
                            pdis = (Functions.DIS(g) | Functions.DEM(g,Functions.GeneAggLOF(g))) & sp # get set of cells from match set with 2ndry disrupted
                            gdeps,nomatch = Functions.ProfPairDist(target[0],pdis,q,Encyclopedia.deps,Encyclopedia.null_trim) # get Encyclopedia.deps for these cells
                            fig = plt.figure(figsize=(3,4))
                            width = 0.02
                            plt.violinplot((match),showmeans=True, showextrema=False)
...
# get all GOF mutations
                    ...
                    GOF = Encyclopedia.muts.loc[(Encyclopedia.muts['LikelyGoF']) &
                                 (Encyclopedia.muts['VariantInfo'] == 'MISSENSE')][['ModelID','HugoSymbol','ProteinChange']]
                    GOF['mutation'] = GOF['HugoSymbol']+GOF['ProteinChange']
                    del GOF['HugoSymbol']
                    del GOF['ProteinChange']
                    
# get those cells in profile match that harbour GOF Encyclopedia.muts
...
# count occurences of the unique 
                    ...
                    for m in uGOF:
                        ...
                        if nm > n_limit:
                            gcount[m] = nm
                    gcount =   dict(sorted(gcount.items(), key = lambda x: x[1], reverse = True))
                    print(f'{len(gcount)} genes have GOF mutation in > {(Encyclopedia.frac_limit)*100}% of the matching cell lines')
...
# go through genes calcualting t-test
                    ...
                    for g in gcount:
                        ...
                        # print(f'Gene {g} is disrupted in {gcount[g]}/{len(sp)} matching cells')
                        cmut = Functions.DEM(GOFgene,Functions.GeneAggGOF(GOFgene)) & sp # cells in matched set with m defined GOF mutation
                        if cmut == set():
                    ...
                    if len(gsig) == 0:
                        ...
                        for g in gsig:
                            ...
                            if tt == 'high': # add this as a positive factor
                                factors.append(f' /{g}:{Functions.GeneAggGOF(g)} ')
                            else :
                                factors.append(f' (# ^  /{g}:{Functions.GeneAggGOF(g)}) ')
                            nfact += 1
                            
                            pdis = sp & Functions.DEM(g,Functions.GeneAggGOF(g))# get set of cells from match set with 2ndry disrupted
                            gdeps,nomatch = Functions.ProfPairDist(target[0],pdis,q,Encyclopedia.deps,Encyclopedia.null_trim) # get Encyclopedia.deps for these cells
                            fig = plt.figure(figsize=(3,4))
                            width = 0.02
                            plt.violinplot((match),showmeans=True, showextrema=False)
...
# gather all CNV deletions in profile matching cells
                    ...
                    for c in p:
                        if c in Encyclopedia.Cdict:
                            del_all += Encyclopedia.Cdict[c].cnvdel
                            del_loc_all += list(set(Encyclopedia.hgnc[Encyclopedia.hgnc.index.isin(Encyclopedia.Cdict[c].cnvdel)]['location_sortable']))
...
# find frequent deleted loci
                    ...
                    print(f'{len(locdel_count)} loci contain genes that are deleted in > {(Encyclopedia.frac_limit)*100}% of the matching cell lines')
                    ...
                    print(f'{len(gdel_count)} individual genes are deleted in > {(Encyclopedia.frac_limit)*100}% of the matching cell lines')
...
# gather all CNV amplifications in profile matching cells
                    ...
                    for c in p:
                        if c in Encyclopedia.Cdict:
                            amp_all += Encyclopedia.Cdict[c].cnvamp
                            amp_loc_all += list(set(Encyclopedia.hgnc[Encyclopedia.hgnc.index.isin(Encyclopedia.Cdict[c].cnvamp)]['location_sortable']))
...
# find frequent amplified loci
                    ...
                    print(f'{len(locamp_count)} loci contain genes that are amplified in > {(Encyclopedia.frac_limit)*100}% of the matching cell lines')
                    ...
                    print(f'{len(gamp_count)} genes are amplified in > {(Encyclopedia.frac_limit)*100}% of the matching cell lines')
...
# go through deletions calculating t-test
                    ...
                    for g in gdel_count:
                        # print(f'Gene {g} is disrupted in {gcount[g]}/{len(sp)} matching cells')
                        cmut = Functions.DEL(g) & sp # cells in matched set with m gene deletion
                        if cmut == set():
...
# IF MULTIPLE SIGNIFICANT GENES SHARE LOCUS - MAKE LOCUS FACTOR NOT GENES          
                        ...
                        loci = Functions.Locify(list(gsig)) # cluster genes into loci
                        ...
                        for l in loci:
                            if len(loci[l]) > 1: # cluster of deletions
                                ...
                                for g in loci[l]:
                                    ...
                                    print(f'Deletion of {g} at {Encyclopedia.hgnc.loc[g]["location"]} is highly significantly over-represented - p = {abs(gsig[g]):.4} d = {gD[g]:.4}\n',
                                          f'in profile matching cells with {tt} dependency on {target[0]}')
                                ...
                                pdis = sp & Functions.LDEL('chr'+l)# get set of cells from match set with 2ndry locus deletion
                                gdeps,nomatch = Functions.ProfPairDist(target[0],pdis,q,Encyclopedia.deps,Encyclopedia.null_trim) # get Encyclopedia.deps for these cells
                                fig = plt.figure(figsize=(3,4))
                                width = 0.02
                                plt.violinplot((match),showmeans=True, showextrema=False)
                                ...
                                pdis = sp & Functions.DEL(g)# get set of cells from match set with 2ndry disrupted
                                gdeps,nomatch = Functions.ProfPairDist(target[0],pdis,q,Encyclopedia.deps,Encyclopedia.null_trim) # get Encyclopedia.deps for these cells
                                fig = plt.figure(figsize=(3,4))
                                width = 0.02
                                plt.violinplot((match),showmeans=True, showextrema=False)
...
# go through deletions calculating t-test
                    ...
                    for g in gamp_count:
                        # print(f'Gene {g} is disrupted in {gcount[g]}/{len(sp)} matching cells')
                        cmut = Functions.AMP(g) & sp # cells in matched set with m gene deletion
                        if cmut == set():
...
# IF MULTIPLE SIGNIFICANT GENES SHARE LOCUS - MAKE LOCUS FACTOR NOT GENES          
                        ...
                        for l in loci:
                            if len(loci[l]) > 1: # cluster of amplifications
                                ...
                                for g in loci[l]:
                                    ...
                                    print(f'Amplification of {g} at {Encyclopedia.hgnc.loc[g]["location"]} is highly significantly over-represented - p = {abs(gsig[g]):.4} d = {gD[g]:.4}\n',
                                          f'in profile matching cells with {tt} dependency on {target[0]}')
                                ...
                                pdis = sp & Functions.LAMP('chr'+l)# get set of cells from match set with 2ndry locus amplification
                                gdeps,nomatch = Functions.ProfPairDist(target[0],pdis,q,Encyclopedia.deps,Encyclopedia.null_trim) # get Encyclopedia.deps for these cells
                                fig = plt.figure(figsize=(3,4))
                                width = 0.02
                                plt.violinplot((match),showmeans=True, showextrema=False)
                ...
                if t != '' and t[0].upper() == 'Y':
                    ...
                    n_limit = len(sp)*Encyclopedia.frac_limit # min no of cells for acceptance
...
# gather all OVER-EXPRESSED genes  in profile matching cells
                    ...
                    for c in p:
                        if c in Encyclopedia.Cdict:
                            over_all += Encyclopedia.Cdict[c].over
                    ...
                    print(f'{len(over_count)} genes are over-expressed in > {(Encyclopedia.frac_limit)*100}% of the matching cell lines')
...
# gather all UNDER-EXPRESSED genes  in profile matching cells
                    ...
                    for c in p:
                        if c in Encyclopedia.Cdict:
                            under_all += Encyclopedia.Cdict[c].under
                    ...
                    print(f'{len(under_count)} genes are under-expressed in > {(Encyclopedia.frac_limit)*100}% of the matching cell lines')

                    path_thresh = 0.1

# find pathways that are highly represented amongst the over/under-expressed genes
                    op = Functions.Pathify(over_count)
                    ...
                    up = Functions.Pathify(under_count)
...
# recreate gene list for over-expressed genes in enriched pathways
                    ...
                    over_enrich = set()
                    for i in opaths:
                        over_enrich |= (set(Encyclopedia.paths[i]) & set(over_count))
...
# go through over-expressions calculating t-test
                    ...
                    for g in over_enrich:
                        cmut = Functions.OX(g,-1) & sp # cells in matched set with precalculated over-expression
                        if cmut == set():
                    ...
                    if len(gsig) == 0:
                        ...
                        for g in gsig:
                            ...
                            pdis = sp & Functions.OX(g,-1)# get set of cells from match set with 2ndry over-expressed
                            gdeps,nomatch = Functions.ProfPairDist(target[0],pdis,q,Encyclopedia.deps,Encyclopedia.null_trim) # get Encyclopedia.deps for these cells
                            fig = plt.figure(figsize=(3,4))
                            width = 0.02
                            plt.violinplot((match),showmeans=True, showextrema=False)
...
# recreate gene list for over-expressed genes in enriched pathways
                    ...
                    under_enrich = set()
                    for i in opaths:
                        under_enrich |= (set(Encyclopedia.paths[i]) & set(under_count))
...
# go through under-expressions calculating t-test
                    ...
                    for g in under_enrich:
                        cmut = Functions.UX(g,-1) & sp # cells in matched set with precalculated under-expression
                        if cmut == set():
                    ...
                    if len(gsig) == 0:
                        ...
                        for g in gsig:
                            ...
                            pdis = sp & Functions.UX(g,-1)# get set of cells from match set with 2ndry under-expression
                            gdeps,nomatch = Functions.ProfPairDist(target[0],pdis,q,Encyclopedia.deps,Encyclopedia.null_trim) # get Encyclopedia.deps for these cells
...
# make base sets from factors 
                    ...
                    for f in factors:
                        pbase.append(eval(Functions.prof_parse(f)))
...
# generate combinations 
                    ...
                    psets.append(eval(Functions.prof_parse(froot)))
                    ...
                    with mp.Pool(mp.cpu_count()) as pool:
                        score = pool.starmap_async(Functions.DandCount,
                            [(i,us-i,target[0],Encyclopedia.deps) for i in psets]).get()
                    ...
                    if len(ans) == 0 or ans[0] != 'N':
                        ...
                        for i in range(len(dompfs)):
                            ...
                            if len(s) == 0 or s[0] != 'N':
                                ...
                                f.write(Functions.prof_flatten(dompfs[i]).replace(" ",'')+'\n')
                        f.close()    
...
# =============================================================================
            ...
            Encyclopedia.default_file = 'query_list.csv'
            
            f = input(f'Gene AB pair file [{Encyclopedia.default_file}] : ')
            if f.strip() == '' :
                f = Encyclopedia.default_file
            print(f'Loading {f}')
            ...
            while True:
                ...
                if tiss == '':
                    ...
                    c = Functions.match_keyword(tiss,set(Encyclopedia.info['OncotreeLineage']))
                    if c == '':
                        print(f'cancer type specification {w} not valid')
                        continue
                    elif c == '*':
            ...
            for i in range(len(pairs)):
                gA = pairs[GeneA].iloc[i]
                gB = pairs[GeneB].iloc[i]
                if gA not in Encyclopedia.deps.columns:
                    print(f'No dependency data for {gA} - skipping')
                    continue
                if gB not in Encyclopedia.deps.columns:
                    print(f'No dependency data for {gB} - skipping')
                    continue

                p_mut_1 = Functions.PairPvalue(gA,gB,Encyclopedia.Cdict,Encyclopedia.deps,Encyclopedia.thresh,True,False,False,tmask)
                p_cnv_1 = Functions.PairPvalue(gA,gB,Encyclopedia.Cdict,Encyclopedia.deps,Encyclopedia.thresh,False,True,False,tmask)
                p_exp_1 = Functions.PairPvalue(gA,gB,Encyclopedia.Cdict,Encyclopedia.deps,Encyclopedia.thresh,False,False,True,tmask)
                               
                p_mut_2 = Functions.PairPvalue(gB,gA,Encyclopedia.Cdict,Encyclopedia.deps,Encyclopedia.thresh,True,False,False,tmask)
                p_cnv_2 = Functions.PairPvalue(gB,gA,Encyclopedia.Cdict,Encyclopedia.deps,Encyclopedia.thresh,False,True,False,tmask)
                p_exp_2 = Functions.PairPvalue(gB,gA,Encyclopedia.Cdict,Encyclopedia.deps,Encyclopedia.thresh,False,False,True,tmask)
            ...
            for i in range(len(pairs)):
                ...
                if gA not in Encyclopedia.deps.columns:
                    print(f'No dependency data for {gA} - skipping')
                    continue
                p1 = Functions.PairPvalue(gA,gB,Encyclopedia.Cdict,Encyclopedia.deps,Encyclopedia.thresh,usemuts,usecnv,useexp)    
                ...
                if both:
                    ...
                    p = Functions.PairPvalue(gB,gA,Encyclopedia.Cdict,Encyclopedia.deps,Encyclopedia.thresh,usemuts,usecnv,useexp)    
...
# =============================================================================
        ...
        # loop through targets and check that target name is in list
                for t in target:
                    ...
                    if t not in list(Encyclopedia.deps.columns) :
                        gene_found = False
        ...
        # now find genes
                ...
                if len(target) == 1:
                    ...
                    print(f'Finding hits for either {Functions.Flistor(target)}')
                else:
                    print(f'Finding hits for any of {Functions.Flistor(target)}')
            # set up parallelism
                ...
                results = pool.starmap_async(Functions.GeneHits,
                        [(g,target,Encyclopedia.Cdict,Encyclopedia.deps,Encyclopedia.thresh,Encyclopedia.frac_limit,Encyclopedia.p_limit,usemuts,usecnv,useexp) for g in glist]).get()
                pool.close()
            
            # =============================================================================
            #     
            #     for g in Encyclopedia.Gdict:
            ...
            # =============================================================================
                ...
                #    HitList = sorted(HitList)   
                    print(f'{len(HitList)} hits found for target {Functions.Flistor(target)} with p<={Encyclopedia.p_limit} and mutated in > {100*Encyclopedia.frac_limit}% of tested cell lines')     
                    for h in HitList:
                ...
                if  len(a) == 0 or (len(a) > 0 and a[0] != 'n'):    
                    for i in HitList:
                        ...
                        t1 = Functions.Flistor(target)
                        t2 = f'{i} f = {h.frac:.2f}'
                        Functions.DepPlot2(h.mut_deps,h.notmut_deps,t1,t2,h.p,D)                       
                        # plt.rcParams.update({'font.size': 30})
...
# =============================================================================
        ...
        elif task.lower() == 'Encyclopedia.muts' : 
...
# for all cell lines with mutations in GeneB, get the dependency value for GeneA
                ...
                if (geneB in Encyclopedia.Gdict) and len(Encyclopedia.Gdict[geneB].cells) > 0:
                       
                    print(f'GeneB {geneB} has disruptive mutations in {len(Encyclopedia.Gdict[geneB].cells)} cell lines')
...
# =============================================================================
                    if len(Encyclopedia.muts) == 0:
                        Encyclopedia.default_mut_dat_file = 'OmicsSomaticMutations_GoFLoFUpdated.csv'
                        f = input(f'Cell line mutation file [{Encyclopedia.default_mut_dat_file}] : ')
                        if f.strip() == '' :
                            f = Encyclopedia.default_mut_dat_file
                        print(f'Loading {f}')
                        
                        Encyclopedia.muts = pd.read_csv(f)      
                        
                        Encyclopedia.default_mut_dat_file = f
                    
# =============================================================================
# get data for all  mutations of this Gene
# =============================================================================
                    mPosGeneB = []
                    for i in Encyclopedia.muts.index:
                        if Encyclopedia.muts.iloc[i]['HugoSymbol'] == geneB:
                            if Encyclopedia.muts.iloc[i]['VariantInfo'] in LOF_Muts: # a loss of function mutation
                                p = Encyclopedia.muts.iloc[i]['ProteinChange']
                                c = Encyclopedia.muts.iloc[i]['ModelID']
                                if c not in Encyclopedia.deps.index:  # cant do anything if mutation is in cell line lacking dependency data
                                    continue
                                t = Encyclopedia.muts.iloc[i]['Transcript']
                                if p != '' and p !='NA' and not pd.isna(p):  # catch poorly annotated splice disruptions
                                    pos = int(re.findall(r'\d+',p)[0])
                                    d = Encyclopedia.deps.loc[c][geneA]
                                    v = Encyclopedia.muts.iloc[i]['VariantInfo']
                    ...
                    trans = Functions.most_frequent([j.trans for j in mPosGeneB])
...
# get genetic profile to use
            ...
            while True:
                print('\nAvailable predefined genetic profiles :')
                for gp in sorted(Encyclopedia.profiles):
                    print(gp)
...
# generate normalisation set
                ...
                pnorm = list(eval(Functions.prof_parse('#')))
...
# =============================================================================
        ...
        elif task.lower() == 'Encyclopedia.expr' : # do the expression analysis 
...
# scatter plot of expression for all genes v. dependency
                ...
                if t not in Encyclopedia.expr.columns[1:] :
                    print(f'{t} not valid gene name')
                    continue
                exp = Encyclopedia.expr[t]
                mexp = exp.mean()
                emod = exp.mode()[0]
                emed = exp.median()
                emin = exp.min()
...
# make distribution
                ...
                exp = Encyclopedia.expr[['ModelID',t]]
                cnv = Encyclopedia.cnvs[['ModelID',t]]
                exp_cnv = pd.merge(exp,cnv, on = 'ModelID')
                Encyclopedia.deps.reset_index(inplace = True)
                dep = Encyclopedia.deps[['ModelID',t]]
